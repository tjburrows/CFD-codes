//	Lid-driven Cavity with explicit, central difference Artificial Compressibility method
//	See Ferziger Computational Methods for Fluid Dynamics, section 7.4.3
//	Travis Burrows
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
#define N           50          //  Number of points to discretize equations in X and Y (including BCs)
...
#pragma omp parallel
        {
            ...
            for (int i = 1+tid; i < N-1; i+=THREADS){
                for (int j = 1; j < N-1; j++){
                    U[LU(i,j,N)] = Up[LU(i,j,N)]
                                   + (dt/(Re * P2(dx))) * (Up[LU(i+1,j,N)] + Up[LU(i-1,j,N)] + Up[LU(i,j+1,N)] + Up[LU(i,j-1,N)] - 4.0 * Up[LU(i,j,N)])
                                   - (dt/(2.0*dx)) * (Pp[LU(i+1,j,N)] - Pp[LU(i-1,j,N)])
                                   - (dt * Up[LU(i,j,N)] / dx) * (Up[LU(i+1,j,N)] - Up[LU(i-1,j,N)])
                                   - (dt * Vp[LU(i,j,N)] / (2.0 * dx)) * (Up[LU(i,j+1,N)] - Up[LU(i,j-1,N)])
                                   - (dt * Up[LU(i,j,N)] / (2.0 * dx)) * (Vp[LU(i,j+1,N)] - Vp[LU(i,j-1,N)]);

                    V[LU(i,j,N)] = Vp[LU(i,j,N)]
                                   + (dt/(Re * P2(dx))) * (Vp[LU(i+1,j,N)] + Vp[LU(i-1,j,N)] + Vp[LU(i,j+1,N)] + Vp[LU(i,j-1,N)] - 4.0 * Vp[LU(i,j,N)])
                                   - (dt/(2.0*dx)) * (Pp[LU(i,j+1,N)] - Pp[LU(i,j-1,N)])
                                   - (dt * Vp[LU(i,j,N)] / dx) * (Vp[LU(i,j+1,N)] - Vp[LU(i,j-1,N)])
                                   - (dt * Up[LU(i,j,N)] / (2.0 * dx)) * (Vp[LU(i+1,j,N)] - Vp[LU(i-1,j,N)])
                                   - (dt * Vp[LU(i,j,N)] / (2.0 * dx)) * (Up[LU(i+1,j,N)] - Up[LU(i-1,j,N)]);
                    P[LU(i,j,N)] = Pp[LU(i,j,N)] - (dt / (2.0 * dx * Beta)) * (Up[LU(i+1,j,N)] - Up[LU(i-1,j,N)] + Vp[LU(i,j+1,N)] - Vp[LU(i,j-1,N)]);
                }
            }
        }
